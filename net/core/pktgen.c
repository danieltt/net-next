 ...
 *
 * Receiver support by Daniel Turull <daniel.turull@gmail.com>
 */
#include <linux/sys.h>
...
int pktgen_call(struct sk_buff *skb);
...
static int pgrx_show(struct seq_file *seq, void *v)
{
	...
	for_each_online_cpu(cpu) {
		...
		seq_printf(seq,"\tWorktime %llu us \n",work_time_us);
	...
	seq_printf(seq,"Start: %llu us\t Stop: %llu us\t Worktime  %llu us\n",
...
static int pktgen_add_rx(const char *ifname)
{
	...
	if(!pg_initialized){
	  	/* grab packets from dev.c/netif_receive_skb() */
		set_pktgen_hook(pktgen_call);
 ...
 */
static int inline throughput_data(ktime_t now)
{
	if(unlikely(__get_cpu_var(pktgen_rx_data).rx_packets==0))
		__get_cpu_var(pktgen_rx_data).start_time=now;
	
	__get_cpu_var(pktgen_rx_data).last_time=now;

	return 0;
}
/* Called at interrupt time from dev.c/netif_receive_skb() */
int pktgen_call(struct sk_buff *skb)
{
	/*check magic*/
